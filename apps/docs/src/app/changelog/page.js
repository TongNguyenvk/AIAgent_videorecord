...
export default async function ChangelogPage() {
    ...
    return (<>
      ...
      <div className="space-y-10">
        {entries.map((entry) => (<section key={entry.version}>
            <h2 className="mb-4 text-lg font-semibold text-neutral-900 dark:text-neutral-100">
              {entry.version}
            </h2>
            {entry.sections.map((section) => (<div key={section.heading} className="mb-4">
                <h3 className="mb-2 text-sm font-medium text-neutral-500 uppercase tracking-wider dark:text-neutral-400">
                  {section.heading}
                </h3>
                <ul className="list-disc space-y-1 pl-5 text-sm">
                  {section.items.map((item, i) => (<li key={i} className="text-neutral-600 dark:text-neutral-400">
                      {item}
                    </li>))}
                </ul>
              </div>))}
