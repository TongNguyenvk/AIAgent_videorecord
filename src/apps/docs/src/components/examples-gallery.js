...
function ExampleCard({ example, playing, onPlay, }) {
    ...
    return (<div>
      <div role={isGif ? undefined : "button"} tabIndex={isGif ? undefined : 0} aria-label={isGif ? undefined : `Play ${example.name} demo video`} className="relative cursor-pointer overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-800" onClick={handleClick} onKeyDown={isGif ? undefined : handleKeyDown}>
        {isGif ? (<img src={example.video} alt={`${example.name} demo`} className="aspect-video w-full object-cover"/>) : (<>
            <video ref={videoRef} src={example.video} poster={`/examples/${example.name}.png`} preload="none" muted playsInline onEnded={handleEnded} className="aspect-video w-full object-cover"/>
            {!playing && (<div className="absolute inset-0 flex items-center justify-center transition-opacity hover:opacity-80" aria-hidden="true">
                <PlayIcon />
              </div>)}
          </>)}
      </div>
      <div className="mt-2 flex items-baseline justify-between gap-2">
        ...
        <a href={`${GITHUB_BASE}/${example.name}`} target="_blank" rel="noopener noreferrer" className="shrink-0 text-xs text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300">
          Source
        </a>
      </div>
...
export function ExamplesGallery() {
    ...
    return (<div className="grid gap-5 sm:grid-cols-2">
      {examples.map((ex) => (<ExampleCard key={ex.name} example={ex} playing={activeVideo === ex.name} onPlay={() => handlePlay(ex.name)}/>))}
    </div>);
}
