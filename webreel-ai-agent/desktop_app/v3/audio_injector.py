...
async def generate_tts_segments(
    ...
    Uses asyncio.gather for PARALLEL generation (same pattern as OS recorder).
    ...
    if engine == "edge":
        print(f"[TTS] Using Edge TTS engine (voice: {voice}) - PARALLEL mode")
        return await _generate_edge_parallel(tts_script, output_dir, voice, speed)
    else:
        if api_key is None:
        ...
        return _generate_fpt_sequential(tts_script, output_dir, voice, speed, api_key)


async def _generate_edge_parallel(
    tts_script: list[dict[str, str]],
    output_dir: Path,
    voice: str,
    speed: str,
) -> list[AudioSegment | None]:
    """Generate all TTS segments in parallel using asyncio.gather (OS recorder pattern)."""
    import asyncio
    ...
    # Build task list (skip empty texts, keep index mapping)
    tasks = []
    task_indices = []
    for i, item in enumerate(tts_script):
        ...
        print(f"  [TTS] Queued segment {i}: '{text[:50]}...'")
        tasks.append(_generate_speech_async(text, out_path, voice, rate))
        task_indices.append(i)

    print(f"  [TTS] Executing {len(tasks)} TTS requests in parallel...")
    results = await asyncio.gather(*tasks, return_exceptions=True)

    # Map results back to ordered segment list
    segments: list[AudioSegment | None] = [None] * len(tts_script)
    success_count = 0

    for idx, result in zip(task_indices, results):
        if isinstance(result, Exception):
            print(f"  [TTS WARN] Failed segment {idx}: {result}")
        else:
            result.duration_ms = measure_audio_duration_ms(result.audio_path)
            result.start_time = 0.0
            segments[idx] = result
            success_count += 1
            print(
                f"  [TTS] Segment {idx}: "
                f"'{tts_script[idx].get('text','')[:50]}...' -> {result.audio_path.name} "
                f"({result.duration_ms}ms)"
            )

    print(f"  [TTS] Generated {success_count}/{len(tasks)} audio files successfully")
    return segments


def _generate_fpt_sequential(
    tts_script: list[dict[str, str]],
    output_dir: Path,
    voice: str,
    speed: str,
    ...
    """Fallback: sequential generation for FPT.AI."""
    ...
    for i, item in enumerate(tts_script):
        ...
        out_path = output_dir / f"narration_{i:03d}.mp3"
        try:
            seg = generate_speech(text=text, output_path=out_path, voice=voice, speed=speed, api_key=api_key)
            seg.duration_ms = measure_audio_duration_ms(out_path)
            seg.start_time = 0.0
            segments.append(seg)
            print(f"  [TTS] {i+1}/{len(tts_script)}: '{text[:50]}...' -> {out_path.name} ({seg.duration_ms}ms)")
