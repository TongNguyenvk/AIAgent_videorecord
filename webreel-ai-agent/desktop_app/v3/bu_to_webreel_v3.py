"""
Parser V3: browser-use history -> webreel config + tts_script.

Returns TWO outputs in one pass:
1. A schema-v1-compliant webreel config (no post-processing needed)
2. A tts_script list for direct FPT.AI TTS generation

This replaces both bu_to_webreel.py AND ai_reviewer.py.
    ...
    for i, action_item in enumerate(actions):
        ...
        # ===== CLICK =====
            ...
            if selector is not None:  # Explicit None check (skip if no usable selector)
                ...
                # moveTo before click for natural cursor animation
                steps.append({
                    "action": "moveTo",
                    "selector": selector,
                })
            ...
            if text and selector:
                ...
                if should_press_enter:
                    ...
                    if is_google_search:
                        ...
                        steps.append({"action": "pause", "ms": 300, "description": "Wait after first Enter"})
                        steps.append({"action": "key", "key": "Enter", "target": selector})
