...
_SEARCH_STOP_WORDS = {
    ...
    "phia", "phía", "goc", "góc", "tai", "tại", "de", "để", "chon", "chọn",
    "click", "nhan", "nhấn", "go", "gõ", "vao", "vào", "cua", "của", "trong",
}
...
def extract_search_phrases(target_text: str) -> list[str]:
    """Split a UI description into shorter phrases that OCR can actually match."""
    ...
    normalized = re.sub(r"[^0-9A-Za-zÀ-ỹà-ỹ]+", " ", raw.lower()).strip()
    words = [w for w in normalized.split() if len(w) > 1 and w not in _SEARCH_STOP_WORDS]
...
def find_text_coordinates(
    ...
    def _normalize(s: str) -> str:
        ...
        return folded.lower().translate(_OCR_NORM)
...
def find_button_or_link(
    ...
    Returns:
        OCRResult if found, None otherwise.
    """
    for phrase in extract_search_phrases(button_text):
        result = find_text_coordinates(image_b64, phrase, min_confidence=0.6)
        if result:
            return result
...
def find_input_label(
    ...
    """
    for phrase in extract_search_phrases(label_text):
        result = find_text_coordinates(image_b64, phrase)
        if result:
