...
def _looks_like_input_target(target: str | None) -> bool:
    value = (target or "").lower()
    return any(
        token in value
        for token in ("ô", "input", "field", "textbox", "textarea", "search", "tim kiem", "tìm kiếm")
    )


def _resolve_action_selector(locator: VisionLocator, action: ParsedAction, coords) -> str | None:
    selector: str | None = None
    target = action.target or ""
    input_like_target = _looks_like_input_target(target)

    if action.action == "type" or (action.action == "click" and input_like_target):
        selector = extract_input_selector(locator.page, coords.x, coords.y, target)
        if selector and validate_selector(locator.page, selector, coords.x, coords.y, tolerance=180):
            return selector

        fallback = extract_selector_from_coordinates(
            locator.page,
            coords.x,
            coords.y,
    ...
    if input_like_target:
        fallback = extract_input_selector(locator.page, coords.x, coords.y, target)
        if fallback:
            return fallback
...
def _resolve_webreel_bin() -> str:
    """Prefer the monorepo build when available, unless a custom binary is set."""
    monorepo_dist = (
        Path(__file__).resolve().parents[2]
        / "packages" / "webreel" / "dist" / "index.js"
    )
    configured = os.environ.get("WEBREEL_BIN", "").strip()
    generic_values = {"", "webreel", "npx webreel"}

    if monorepo_dist.exists() and configured.lower() in generic_values:
        return f'node "{monorepo_dist}"'

    if configured:
        return configured

    return f'node "{monorepo_dist}"' if monorepo_dist.exists() else "npx webreel"
...
def _simulate_action_in_browser(
    locator,
    action: ParsedAction,
    selector: str | None,
    coords,
    ...
    """
    Replay an action in the Playwright browser so the page state advances
    before we resolve the next action.  This allows Vision AI to see the
    correct page (e.g. search results after pressing Enter).
    """
    page = locator.page

    try:
        if action.action == "navigate" and action.url:
            locator.navigate(action.url)
            ...
            if selector:
                try:
                    ...
                    pass
            elif coords and coords.x >= 0 and coords.y >= 0:
                page.mouse.click(coords.x, coords.y)
                page.wait_for_timeout(300)
            page.keyboard.type(action.text, delay=10)
            page.wait_for_timeout(500)

        elif action.action == "key" and action.key:
            page.keyboard.press(action.key)
            # Wait longer for page navigation (e.g. Enter on search)
            ...
            y_delta = 500 if direction == "down" else -500
            page.mouse.wheel(0, y_delta)
            page.wait_for_timeout(1000)

    except Exception as e:
        print(f"[pipeline] Warning: simulate '{action.action}' failed: {e}")
...
def run_pipeline(
        ...
        for action in actions:
            if action.action in ("click", "type") and action.target:
                ...
                if coords.x >= 0 and coords.y >= 0:
                    selector = _resolve_action_selector(locator, action, coords)

                resolved_actions.append(
                    ResolvedAction(**action.model_dump(), selector=selector, coordinates=coords)
                )

                # --- Simulate action in Playwright so page state advances ---
                _simulate_action_in_browser(locator, action, selector, coords)
            else:
                resolved_actions.append(ResolvedAction(**action.model_dump()))

                # --- Simulate non-vision actions too (navigate, key, scroll) ---
                _simulate_action_in_browser(locator, action, None, None)
    ...
    webreel_bin = _resolve_webreel_bin()
    cmd = f"{webreel_bin} record {video_name}"
    # Pass CHROME_HEADLESS_PATH so webreel uses full Chromium (not stripped
    # chrome-headless-shell which can't render pages after navigation).
    env = {**os.environ}
    chrome_path = os.getenv("CHROME_HEADLESS_PATH")
    if chrome_path:
        env["CHROME_HEADLESS_PATH"] = chrome_path
