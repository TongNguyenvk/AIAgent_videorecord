...
def run_os_pipeline_v3_dual(
    ...
    output_dir: str = "workspace/pipeline_v3_dual",
    ...
    if not skip_tts and narrations:
        ...
        if use_edge:
            ...
            for idx, (narr, result) in enumerate(zip(narrations, tts_results)):
                if isinstance(result, Exception):
                    logger.error(f"  [{narr['index']}] TTS failed: {result}")
                    audio_files.append(None)
                else:
                    audio_files.append({"path": str(result.audio_path), "duration_ms": result.duration_ms})
                    logger.info(f"  [{narr['index']}] -> {result.audio_path.name} ({result.duration_ms}ms)")
        else:
    ...
    if not dry_run:
        ...
        from core.os_planning_agent_v2 import replay_plan_with_recording
    ...
    if enable_dual_output and result["screenshots"]:
        ...
        logger.info(f"  Screenshot map: {screenshot_map}")
